    ...
    for (int i = 0; i < MAX_PRODUCTS; i++) {
        if (products[i].id != 0) {
            fprintf(file, "%d %s %d %d %.2f %.2f\n", products[i].id, products[i].name,
                    products[i].stock, products[i].sold, products[i].price, products[i].total_sales);
        }
    }
    fclose(file);
...
void load_product_info() {
    ...
    while (fscanf(file, "%d %49s %d %d %f %f", &products[product_count].id, products[product_count].name,
                  &products[product_count].stock, &products[product_count].sold, &products[product_count].price,
                  &products[product_count].total_sales) != EOF) {
        product_count++;
    }
    fclose(file);
...
void view_inventory() {
    ...
    for (int i = 0; i < MAX_PRODUCTS; i++) {
        if (products[i].id != 0) {
            ...
            total_stock += products[i].stock + products[i].sold;  // 총 입고 수량을 계산
...
int main() {
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 5: save_product_info(); break;
            case 6: load_product_info(); break;
